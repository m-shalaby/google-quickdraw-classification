...
static inline void reduceNoise(void) {
    ...
    for (int i = 0; i < REDUCTION_ITERATIONS; ++i) {
        ...
        int bottom = row    + PIXEL_RADIUS_SIZE,
        ...
        top = top < 0 ? 0 : top;
        int counter = 0;
        __attribute__((__vector_size__(sizeof(char) * 128))) char segment = {0};
        for (int offset = top; offset <= bottom; ++offset) segment += canvas[offset];
        for (int offset = left; offset <= right; ++offset) counter += segment[offset];
