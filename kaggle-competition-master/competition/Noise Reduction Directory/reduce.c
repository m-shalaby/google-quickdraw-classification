...
#endif
#ifndef MAIN_FRAME_MAXIMA_AREA
#define MAIN_FRAME_MAXIMA_AREA 0
#endif
#ifndef MAIN_FRAME_MINIMA_AREA
#define MAIN_FRAME_MINIMA_AREA 0
#endif
...
static inline int setMainSubFrameBoundingBox(struct Image * restrict image) {
    ...
    for (int i = 0; i < FRAME_IDENTIFICATION_ITERATIONS; ++i) {
        ...
        int bottom = row, left = column, right = column, top = row;
        do if (!growBoundingBox(image, &bottom, bottomBound, &left, leftBound, &right, rightBound, &top, topBound)) break;
        while ((bottom - top + 1) * (right - left + 1) < MAIN_FRAME_MAXIMA_AREA);
        ...
        int frameArea = getFrameArea(&newFrame);
        if (frameArea < MAIN_FRAME_MINIMA_AREA) continue;
        
        #pragma omp critical
        if (frameArea > getFrameArea(&currentFrame)) { currentFrame = newFrame; } else if (frameArea == getFrameArea(&currentFrame)) {

            int currentFramePixelsCount = 0;
            int newFramePixelsCount = 0;
            for (int row = 0; row < currentFrame.rowsCount; ++row)
            for (int column = 0; column < currentFrame.columnsCount; ++column) currentFramePixelsCount += image->imageMatrix[row][column] ? 1 : 0;
            for (int row = 0; row < newFrame.rowsCount; ++row)
            for (int column = 0; column < newFrame.columnsCount; ++column) newFramePixelsCount += image->imageMatrix[row][column] ? 1 : 0;
            if (newFramePixelsCount > currentFramePixelsCount) currentFrame = newFrame;
